...
async def upload_file(
    ...
    file_url: str = Body(..., description="文件上传后的 URL"),
    ...
    try:
        ...
        file_name = "_".join(parts[:-1]) + f".{ext_part}" if len(parts) > 1 else file_name

        knowledge_file_id = await KnowledgeFileService.create_knowledge_file(
            file_name=file_name,
